...
|---|---|
| **Runtime** | Bun |
| **Queue and logs** | Valkey |
| **User agent** | `Fluxhook/1.0` |

## How it works

When a significant event occurs in Flux Drop (for example, a file is uploaded or downloaded), a message is queued for the Webhook Service. The worker then:
...
The service keeps a rotating log of the last **100 delivery attempts** (retained for 7 days) per user in Valkey. This log tracks the `endpoint`, `event`, `status` code, and `duration_ms` to help you debug any integration issues.